...
export class Boardroid extends Kernel {
  ...
  constructor({ filters = null, config_port, no_device = 1, socket = false }: ConstructorParams = {}) {
    ...
    Devices.add(this as unknown as Kernel);
  }
  ...
  set price(value: number) {
    let price = parseFloat(value as unknown as string);
  ...
  #internalMessageConnected(message: MessageSerial) {
  ...
  #typeCoin(code: string) {
    ...
    let coin = null;
    for (const key in pennies) {
      if (!pennies[key as keyof typeof pennies].includes(code)) continue;
    ...
    return [types[coin as keyof typeof types], coin];
  }

  #isCoin(type_money: string) {
    return ['g50', 'c50', 'p1', 'p2', 'p5', 'p10', 'p20'].includes(type_money);
  }

  #typeBanknote(code: string) {
    const banknotes = {
      p20: ['80', '90', 'a0', 'b0'],
      p50: ['81', '91', 'a1', 'b1'],
      p100: ['82', '92', 'a2', 'b2'],
    ...
    for (const key in banknotes) {
      if (!banknotes[key as keyof typeof banknotes].includes(code)) continue;
    ...
    return [types[banknote as keyof typeof types], banknote];
  }

  #typeBanknoteValue(key: string) {
    const types = {
      p20: { value: 20, name: '20 pesos' },
      ...
      p1000: { value: 1000, name: '1000 pesos' },
    };
  ...
  #isBanknote(type_money: string) {
    return ['p20', 'p50', 'p100', 'p200', 'p500', 'p1000'].includes(type_money);
  }

  #isBanknoteOut(type_money: string) {
  ...
  #countMoney(type_money: string | null, where_gone: string, type: string) {
    if (!type_money) return;

    let _dispatch = true;
    if (this.#isCoin(type_money) && type === 'coin') {
      if (typeof this.coins.tubes[type_money as keyof typeof this.coins.tubes] === 'undefined') return;

      if (where_gone === 'tube') {
        this.coins.tubes[type_money as keyof typeof this.coins.tubes] += 1;
      } else if (where_gone === 'box') {
        this.coins.box[type_money as keyof typeof this.coins.box] += 1;
      }
      ...
      this.coins.totals[type_money as keyof typeof this.coins.totals] += inserted;
      this.__money_session.inserted += inserted;
      this.coins.total += inserted;

      const cValue = this.#typeCoinValue(type_money);
      if (cValue) {
        this.dispatch('money:inserted', {
          type: 'coin',
          money: cValue, // {value:number,name:string}
          where: where_gone,
        });
      }
    } else if (this.#isBanknote(type_money) && type === 'banknote') {
      if (typeof this.banknotes.recycler[type_money as keyof typeof this.banknotes.recycler] === 'undefined') return;

      if (where_gone === 'recycler') {
        this.banknotes.recycler[type_money as keyof typeof this.banknotes.recycler] += 1;
      } else if (where_gone === 'stacker') {
        this.banknotes.stacker[type_money as keyof typeof this.banknotes.stacker] += 1;
      }

      const inserted = parseInt(type_money.slice(1));
      this.banknotes.totals[type_money as keyof typeof this.banknotes.totals] += inserted;
      this.__money_session.inserted += inserted;
      this.banknotes.total += inserted;

      const cValue = this.#typeBanknoteValue(type_money);
      if (cValue) {
        this.dispatch('money:inserted', {
          type: 'banknote',
          money: cValue, // {value:number,name:string}
      ...
      if (typeof this.banknotes.out[type_money.replace('r', 'p') as keyof typeof this.banknotes.out] === 'undefined')
        return;

      this.banknotes.out[type_money.replace('r', 'p') as keyof typeof this.banknotes.out] += 1;
      const retired = parseInt(type_money.slice(1));
      this.__money_session.retired += retired;
      this.banknotes.recycler[type_money.replace('r', 'p') as keyof typeof this.banknotes.recycler] -= 1;
  ...
  #internalMessageInsertedCoin(code: string[], message: MessageSerial) {
  ...
  #internalMessageInsertedBanknote(code: string[], message: MessageSerial) {
    const _decimal = parseInt(code[2], 16);
  ...
  #internalMessageConfigCoinPurse(code: string, message: MessageSerial) {
  ...
  #internalMessageConfigBillPurse(code: string[], message: MessageSerial) {
  ...
  #internalMessageSetCoinsTubes(code: string[], message: MessageSerial) {
    message.no_code = 6;
  ...
  #internalMessageSetBillsRecycler(code: string[], message: MessageSerial) {
    message.no_code = 7;
  ...
  #internalMessageStatusBanknoteScrow(code: string, message: MessageSerial) {
    const _decimal = parseInt(code, 16);
    if (_decimal === 1) {
  ...
  #internalMessageDispensedBanknotes(code: string[], message: MessageSerial) {
    const [b20, b50, b100, b200, b500, b1000] = [
      parseInt(code[2], 16),
      parseInt(code[3], 16),
      parseInt(code[4], 16),
  ...
  #internalMessageCoinsDispensed(message: MessageSerial) {
    ...
    // @ts-expect-error 2nd param expected to be any
    this.dispatchAsync('session:money-dispensed', obj, 500);

    return message;
  }

  #internalMessageDispenseProduct(code: string, message: MessageSerial) {
    const _decimal = parseInt(code, 16);
    if (_decimal === 1) {
  ...
  #internalMessageDoorStatus(code: string, message: MessageSerial) {
    let status = 'closed';
    if (code === 'db') {
  ...
  #internalMessageTemperatureStatus(code: string[], message: MessageSerial) {
  ...
  #internalMessageRelayStatus(code: string, message: MessageSerial) {
    const _decimal = parseInt(code, 16);
    let status = 'unknown';
    if (_decimal === 1) {
  ...
  #internalMessageCardReader(code: string[], message: MessageSerial) {
    // [2] => status
    // [3] => selection
    // [4] => selection_aux
    // [5] => time dispense
  ...
  serialMessage(code: string[] | Uint8Array<ArrayBufferLike> | string | ArrayBuffer) {
    code = this.fixHexArray(code as string[] | number[]);

    let message: MessageSerial = {
      code,
      name: null,
      description: null,
      request: null,
    ...
    switch (code[1]) {
      ...
      case 'd6':
        ...
        message = this.#internalMessageCoinsDispensed(message);
        break;
      case 'd7':
        message.request = `dispense`;
  ...
  async coinPurseConfigure({ enable = false, high = 0xff, low = 0xff } = {}) {
  ...
  async dispense(
    ...
    selection = parseInt(selection as unknown as string, 10);
  ...
  #percentageTestMatrix({
    dispensed = null,
    limit = 80,
  }: {
    dispensed?: null | DispenseResponse[];
    limit?: number;
  } = {}) {
  ...
  #internalBanknotesChangeToReturnWithIct(change: number) {
    ...
    const maxBanknotesAvailable =
      this.banknotes.recycler[`p${banknotes[banknoteIndex]}` as keyof typeof this.banknotes.recycler]; // Número máximo disponible
    ...
    data.banknotes[banknoteName as keyof typeof data.banknotes] = toDispense;
    data.pending = parseFloat((change - toDispense * banknoteValue).toFixed(2));
    return data;
  }

  #internalBanknotesChangeToReturnWithOther(change: number) {
    const data = {
      banknotes: { $_20: 0, $_50: 0, $_100: 0, $_200: 0, $_500: 0, $_1000: 0 },
      pending: change,
      will_dispense: false,
    ...
    const calculateBanknotes = (denomination: number, key: string) => {
      if (this.banknotes.recycler[key as keyof typeof this.banknotes.recycler] > 0) {
        const maxBanknotes = Math.floor(data.pending / denomination);
        const banknotesToUse = Math.min(
          maxBanknotes,
          this.banknotes.recycler[key as keyof typeof this.banknotes.recycler]
        );
        data.banknotes[`$_${denomination}` as keyof typeof data.banknotes] = banknotesToUse;
  ...
  #internalBanknotesChangeToReturn(change: number) {
    if (!this.hasRecycler) {
      return {
        banknotes: { $_20: 0, $_50: 0, $_100: 0, $_200: 0, $_500: 0, $_1000: 0 },
        pending: change,
  ...
  #internalCoinsChangeToReturn(change: number) {
    const data = {
    ...
    const calculateChange = (denomination: number, tubeKey: string, denominationName: null | string = null) => {
      if (this.coins.tubes[tubeKey as keyof typeof this.coins.tubes] > 0) {
        if (denominationName === null) {
          denominationName = '$_' + denomination;
        }
        const maxCoins = Math.floor(data.pending / denomination);
        const coinsToUse = Math.min(maxCoins, this.coins.tubes[tubeKey as keyof typeof this.coins.tubes]);
        data.coins[denominationName as keyof typeof data.coins] = coinsToUse;
  ...
  async #internalReturnChange(custom_change: null | number = null) {
  ...
  override async serialCorruptMessage(
