...
export class Kernel extends Dispatcher {
   ...
   */
  constructor({ filters = null, config_port = null, no_device = 1, device_listen_on_port = 1 } = {}) {
    super();
    ...
    if (
      ...
      (typeof device_listen_on_port === 'number' || typeof device_listen_on_port === 'string')
    ) {
  ...
  async #runQueue() {
    ...
    this.__internal__.serial.last_action = first.action;
    this.dispatch('serial:sent', {
      action: first.action,
      bytes: first.bytes,
    });
