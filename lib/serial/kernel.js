...
export class Kernel extends Dispatcher {
    __internal__ = {
        ...
        dispense: {
            ...
            counter: 0,
            limit_counter: 20,
            elevator_locking: false,
            elevator_locking_time: 60,
            elevator_locking_interval: 0,
        },
    }
    ...
    internalDispenseElevatorProcess() {
        if (this.__internal__.dispense.elevator_locking_interval) return;
        const this1 = this;
        this.__internal__.dispense.status = 'elevator-locked';
        this.__internal__.dispense.elevator_locking = true;
        this.__internal__.dispense.elevator_locking_time = 60;
        this.dispatch('internal:jofemar-collect', {});

        this.__internal__.dispense.elevator_locking_interval = setInterval(() => {
            this.dispatch('dispensing:withdrawal', {
                elevator: true, seconds: this1.__internal__.dispense.elevator_locking_time,
                description: `Please recall products from elevator`,
            });
            this1.__internal__.dispense.elevator_locking_time -= 1;
            if (this1.__internal__.dispense.elevator_locking_time <= 0) {
                clearInterval(this1.__internal__.dispense.elevator_locking_interval);
                this1.__internal__.dispense.elevator_locking_interval = 0;
                this1.__internal__.dispense.elevator_locking = false;
                //this1.__internal__.dispense.status = undefined;
                // todo: redispense the same product
            }
        }, 1e3);


        // this.collect();
        // if (this1.#_status_waiting_for_sense) {
        //     clearInterval(this1.#_status_waiting_for_sense);
        // }
        // this1.#_status_waiting_for_sense = undefined;
        // // this.clearSenseProduct();
        // this.#_status_dispense = 'busy';
        // this1.dispatch('dispensing:withdrawal', {
        //     elevator: true,
        //     description: dd.__('Please recall products from elevator'),
        // });
        //
        // return new Promise((resolve) => {
        //     setTimeout(() => {
        //         this1.dispatch('pickupToContinue', {starting: true, seconds: null});
        //         clearInterval(this1.#_waiting_withdrawal_interval);
        //         this1.#_waiting_withdrawal_interval = undefined;
        //
        //         this1.#_waiting_withdrawal_interval = setInterval(() => {
        //             let b = this.#_waiting_withdrawal_timer / 1000;
        //             this1.#_waiting_withdrawal_timer -= 1000;
        //             this1.dispatch('pickupToContinue', {starting: false, seconds: b});
        //             if (this1.#_waiting_withdrawal_timer <= 0) {
        //                 // this1.continueDispensing(); // when the products are being removed
        //                 clearInterval(this1.#_waiting_withdrawal_interval);
        //                 this1.#_waiting_withdrawal_interval = undefined;
        //                 this1.#_waiting_withdrawal_timer = 0;
        //                 this1.#_waiting_aux = false;
        //                 this1.#_waiting_withdrawal = false;
        //                 this1.dispatch('dispensing:tryingAgain', {
        //                     same_selection: true,
        //                 });
        //                 this1.#_status_dispense = undefined;
        //                 this1.#_count_timeout_sense = 0;
        //                 this1.#tryDispenseAgain();
        //                 resolve();
        //             }
        //         }, 1e3)
        //     }, 4e2);
            ...
            this.__internal__.interval.waiting_sense = setInterval(() => {
                switch (this1.__internal__.dispense.status) {
                    ...
                    case 'elevator-locked':
                        if (!this1.__internal__.dispense.elevator_locking) {
                            this.internalDispenseElevatorProcess();
                        }
                        break;
                }
