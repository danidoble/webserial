...
export class Kernel extends Dispatcher {
    ...
    constructor(
        ...
        if (device_listen_on_port && (typeof device_listen_on_port === 'number' || typeof device_listen_on_port === 'string')) {
    ...
    set listenOnPort(port) {
        port = parseInt(port);
        if (isNaN(port) || port < 1 || port > 255) throw new Error('Invalid port number');
    ...
    async #runQueue() {
        ...
        if (first.action === 'dispense') {
            limit_response = this.__internal__.dispense.limit_counter * 1000;
            if (this.__internal__.dispense.custom_limit_counter) {
                limit_response = this.__internal__.dispense.custom_limit_counter * 1000;
            }
            limit_response += Math.ceil(limit_response * 0.6);
        }
    ...
    internalDispensingProcess() {
        ...
        console.log(this.__internal__.dispense.counter)
