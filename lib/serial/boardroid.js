...
export class Boardroid extends Core {
    ...
    constructor({
        ...
        this.__internal__.serial.time.sense = 15e3;
        this.__internal__.serial.time.response_general = 4e3;
    ...
    get totalInTubes() {
        return this.#coins.tubes.g50 * 0.5 +
    ...
    get totalInRecycler() {
        return this.#banknotes.recycler.p20 * 20 +
    ...
    set price(value) {
        ...
        this._sale.price = price;
    }

    get price() {
        return this._sale.price;
    }

    get change() {
        if (this._sale.price <= 0 || this.__money_session.inserted <= this._sale.price) return 0;

        return this.__money_session.inserted - this._sale.price;
    }
    ...
    #countMoney(type_money, where_gone) {
        if (!type_money) return;

        let _dispatch = true;
        if (this.#isCoin(type_money)) {
        ...
        } else if (this.#isBanknote(type_money)) {
            if (typeof this.#banknotes.tubes[type_money] === 'undefined') return;
        ...
        } else if (this.#isBanknoteOut(type_money) && where_gone === 'out') {
            if (typeof this.#banknotes.out[type_money] === 'undefined') return;

            this.#banknotes.out[type_money] += 1;
    ...
    #internalMessageInsertedCoin(code, message) {
        ...
        } else if (_decimal >= 112 && _decimal <= 127) { // 70 - 7F
            message.name = `Coin rejected`;
            message.additional.where = 'rejected';
        } else if (_decimal >= 144 && _decimal <= 159) { // 90 - 9F
        ...
        this.#countMoney(message.additional.coin, message.additional.where);
        if (['tube', 'out'].includes(message.additional.where)) {
            this.dispatch('coin-purse:tubes', this.#coins.tubes)
        }
        this.dispatch('coin-purse:event-coin', this.#coins);
    ...
    #internalMessageInsertedBanknote(code, message) {
        ...
        this.#countMoney(message.additional.banknote, message.additional.where);
    ...
    #internalMessageConfigCoinPurse(code, message) {
        ...
        this.dispatch('coin-purse:config', {status: _decimal === 1});
    ...
    #internalMessageConfigBillPurse(code, message) {
        ...
        this.dispatch('banknote-purse:config', {status: _decimalN2 === 1, scrow: _decimalN3 === 1});
        return message;
    }
    ...
    #internalMessageDispensedBanknotes(code, message) {
        ...
        message.description = total_dispensed > 0 ? `Banknotes dispensed by request` : `No banknotes dispensed, recycler empty`;
        message.no_code = 9;
        message.additional = {
            banknotes: {b20, b50, b100, b200, b500, b1000},
            total_dispensed,
    ...
    #internalMessageCoinsDispensed(code, message) {
        ...
        if (isNaN(this._sale.last_change)) {
            this._sale.last_change = 0;
        }
        this.__money_session.retired += this._sale.last_change;
        this.dispatchAsync('session:money-dispensed', {
            type_money: null,
            retired: null,
            finish: false,
            type: 'coins'
    ...
    #internalMessageDoorStatus(code, message) {
        ...
        this.dispatch('status:door', {status})
        return message;
    }
    ...
    #internalMessageRelayStatus(code, message) {
        ...
        this.dispatch('status:relay', {status});
        return message;
    }
    ...
    #internalMessageCardReader(code, message) {
        ...
        } else if (_decimal === 10) { // end of dispense, status of dispense
    ...
    #addToQueueWithChecksum(bytes, event) {
        return this.appendToQueue(this.#boardroidCheckSum(bytes), event);
    }

    #boardroidCheckSum(bytes) {
        bytes[11] = this.serialBoardroidSumHex(bytes);
        bytes.map((value, index) => {
            bytes[index] = this.hexMaker(value);
        });
        return bytes;
    }
    ...
    async coinPurseConfigure({enable = false, high = 'FF', low = 'FF'} = {}) {
        ...
        const txStatus = enable ? '01' : '00';
        const bytes = ["F1", "C1", txStatus, high, low, "00", "00", "00", "00", "00", "F2", "00"];
        await this.#addToQueueWithChecksum(bytes, 'coin-purse:config');
    }
    ...
    async coinPurseDispense({$_50c = 0, $_1 = 0, $_2 = 0, $_5 = 0, $_10 = 0} = {}) {
        if (!this.__coin_purse.available) throw new Error('Coin purse not available');
        if ([$_50c, $_1, $_2, $_5, $_10].some(value => isNaN(value) || typeof value === 'string')) {
            throw new Error('One of the values is not a number');
        }
        if ($_50c < 1 && $_1 < 1 && $_2 < 1 && $_5 < 1 && $_10 < 1) throw new Error('No coins to dispense');
        ...
        let bytes = ["F1", "C6", $_50c, $_1, $_2, $_5, $_10, "00", "00", "00", "F2", "00"];
        await this.#addToQueueWithChecksum(bytes, 'coin-purse:dispense');
    }

    async coinPurseReadTubes() {
        const bytes = ["F1", "C2", "00", "00", "00", "00", "00", "00", "00", "00", "F2", "00"];
        await this.#addToQueueWithChecksum(bytes, 'coin-purse:read-tubes');
    }

    #banknotePurseICTConfigure({enable = false, scrow = false} = {}) {
        const txStatus0 = enable ? 'FF' : '00';
        const txScrow0 = scrow ? 'FF' : '00';
        return ["F1", "C0", txStatus0, txStatus0, txScrow0, txScrow0, "00", "00", "00", "00", "F2", "00"];
    }

    #banknotePurseOtherConfigure({enable = false, scrow = false} = {}) {
        const txStatus0 = enable ? '01' : '00';
        const txScrow0 = scrow ? '01' : '00';
        return ["F1", "C0", txStatus0, txScrow0, "00", "00", "00", "00", "00", "00", "F2", "00"];
    }
    ...
    async banknotePurseConfigure({enable = false, scrow = false} = {}) {
        ...
        return await this.#addToQueueWithChecksum(bytes, 'banknote-purse:config');
    }

    #banknotePurseIctDispense(quantity = 1) {
        if (quantity < 1) throw new Error('No banknotes to dispense');

        quantity = this.decToHex(quantity);
        const currency = this.decToHex((this.__banknote_purse.recycler.banknote).toString());
        return ["F1", "C5", currency, quantity, "00", "00", "00", "00", "00", "00", "F2", "00"];
    }

    #banknotePurseOtherDispense({$_20 = 0, $_50 = 0, $_100 = 0, $_200 = 0, $_500 = 0, $_1000 = 0} = {}) {
        if ([$_20, $_50, $_100, $_200, $_500, $_1000].some(value => isNaN(value) || typeof value === 'string')) {
            throw new Error('One of the values is not a number');
        }
        if ($_20 < 1 && $_50 < 1 && $_100 < 1 && $_200 < 1 && $_500 < 1 && $_1000 < 1) throw new Error('No banknotes to dispense');

        [$_20, $_50, $_100, $_200, $_500, $_1000] = [
            this.decToHex($_20),
            this.decToHex($_50),
            this.decToHex($_100),
            this.decToHex($_200),
        ...
        return ["F1", "C5", $_20, $_50, $_100, $_200, $_500, $_1000, "00", "00", "F2", "00"];
    }
    ...
    async banknotePurseDispense({$_20 = 0, $_50 = 0, $_100 = 0, $_200 = 0, $_500 = 0, $_1000 = 0} = {}) {
        ...
        await this.#addToQueueWithChecksum(bytes, 'banknote-purse:dispense');
    }
    ...
    async banknotePurseAcceptInScrow() {
        ...
        const bytes = ["F1", "C4", "01", "00", "00", "00", "00", "00", "00", "00", "F2", "00"];
        await this.#addToQueueWithChecksum(bytes, 'banknote-purse:banknote-scrow-status');
    }

    async banknotePurseRejectInScrow() {
        if (!this.__banknote_purse.available) throw new Error('Banknote purse not available');

        const bytes = ["F1", "C4", "00", "00", "00", "00", "00", "00", "00", "00", "F2", "00"];
        await this.#addToQueueWithChecksum(bytes, 'banknote-purse:banknote-scrow-status');
    }
    ...
    async banknotePurseSaveMemory(
        ...
        if (channel === null || $_20 === null || $_50 === null || $_100 === null || $_200 === null || $_500 === null || $_1000 === null) {
            throw new Error('One of the values is not defined');
        }

        const bytes = [
            "F1", "C8", this.decToHex(channel), "00",
            this.decToHex($_20),
            this.decToHex($_50),
            this.decToHex($_100),
            this.decToHex($_200),
        ...
        await this.#addToQueueWithChecksum(bytes, 'banknote-purse:save-memory');
    }
    ...
    async banknotePurseReadRecycler() {
        ...
        const bytes = ["F1", "C3", "00", "00", "00", "00", "00", "00", "00", "00", "F2", "B5"];
        return await this.#addToQueueWithChecksum(bytes, 'banknote-purse:read-recycler');
    }

    async cardReaderDisable() {
        const bytes = ["F1", "CD", "00", "00", "00", "00", "00", "00", "00", "00", "F2", "00"];
        return await this.#addToQueueWithChecksum(bytes, 'card-reader:disable');
    }
    ...
    async cardReaderDispense({channel = 1, second_channel = null, sensor = true, seconds = null, price = 0} = {}) {
        ...
        if (!sensor && (seconds === null || seconds <= 0 || seconds > 25.4)) throw new Error('Invalid time to dispense without sensor');

        // get high and low from price, 8 bits each
        const high = this.decToHex(price / 256);
        const low = this.decToHex(price % 256);

        const __channel = this.decToHex(channel + 9);
        let __second_channel = '00';
        if (second_channel) {
            __second_channel = this.decToHex(second_channel + 9);
        }

        let __type = '00';
        if (!sensor) {
            __type = this.decToHex(seconds * 10);
        }

        const bytes = ["F1", "CD", "01", __channel, __second_channel, __type, high, low, "00", "00", "F2", "00"];
        await this.#addToQueueWithChecksum(bytes, 'card-reader:dispense');
    }

    async paymentPursesDisable({coin = true, banknote = true, cardReader = false} = {}) {
        if (coin) await this.coinPurseDisable();
        if (banknote) await this.banknotePurseDisable();
        if (cardReader) await this.cardReaderDisable();
    }

    async paymentPursesEnable({coin = true, banknote = true, scrowBanknote = false} = {}) {
        if (coin) await this.coinPurseEnable();
        if (banknote) await this.banknotePurseEnable({scrow: scrowBanknote});
    }

    async coolingRelayConfigure({enable = false} = {}) {
        const txStatus = enable ? '01' : '00';
        const bytes = ["F1", "CC", txStatus, "00", "00", "00", "00", "00", "00", "00", "F2", "00"];
        return await this.#addToQueueWithChecksum(bytes, 'status:relay');
    }
    ...
    async readTemperature() {
        const bytes = ["F1", "CB", "00", "00", "00", "00", "00", "00", "00", "00", "F2", "00"];
        return await this.#addToQueueWithChecksum(bytes, 'status:temperature');
    }
     ...
     */
    async dispense({channel = 1, second_channel = null, sensor = true, seconds = null} = {}) {
        if (channel < 1 || channel > 80) throw new Error('Invalid channel number');
        if (second_channel && (second_channel < 1 || second_channel > 80 || second_channel === channel)) throw new Error('Invalid second channel number');
        if (!sensor && (seconds === null || seconds <= 0 || seconds > 25.4)) throw new Error('Invalid time to dispense without sensor');

        channel += 9;
        const __channel1 = this.decToHex(channel);

        let __channel2 = '00';
        if (second_channel) {
            second_channel += 9;
            __channel2 = this.decToHex(second_channel);
        }
        ...
        const bytes = this.#boardroidCheckSum(["F1", "C7", __channel1, __channel2, __type, "00", "00", "00", "00", "00", "F2", "00"]);
        return await this.internalDispense(bytes);
    }

    #resetTestMatrix() {
        this.#_is_matrix_test = false;
        this.#_active_engine_testing = 0;
        this.#_percentage_test_matrix = 0;
    }

    #percentageTestMatrix(dispensed = null) {
        this.#_percentage_test_matrix = Math.round((this.#_active_engine_testing * 100) / 80);
        this.dispatch('percentage:test', {percentage: this.#_percentage_test_matrix, dispensed});
    }
    ...
    async testEngines({singleEngine = false} = {}) {
        ...
        const dispensed = [];
        const limit = singleEngine ? 80 : 40;

        for (let i = 1; i <= limit; i++) {
            const tmp = await this.dispense({
                channel: i,
                second_channel: singleEngine ? null : i + 1,
                sensor: false,
                seconds: 0.4,
            });
            dispensed.push(tmp);
            this.#_active_engine_testing = i;
            this.#percentageTestMatrix();
            if (!singleEngine) {
                i++;
            }
        }
        this.#_active_engine_testing = 80;
        this.#percentageTestMatrix(dispensed);
        this.#resetTestMatrix();
    }

    async sendCustomCode(bytes) {
        bytes = this.#boardroidCheckSum(bytes);
    ...
    #internalBanknotesChangeToReturnWithIct(change) {
        ...
        const banknote_name = '$_' + banknotes[this.__banknote_purse.recycler.banknote];
        const banknote = parseInt(banknotes[this.__banknote_purse.recycler.banknote]);
        const toDispense = parseInt((change / banknote).toString());
        const data = {
        ...
        data.banknotes[banknote_name] = toDispense;
        data.pending = change % banknote;
        return data;
    }
    ...
    #internalBanknotesChangeToReturnWithOther(change) {
        ...
        data.banknotes.$_1000 = parseInt((change / 1000).toString());
        data.pending = change % 1000;

        data.banknotes.$_500 = parseInt((data.pending / 500).toString());
        data.pending = data.pending % 500;

        data.banknotes.$_200 = parseInt((data.pending / 200).toString());
        data.pending = data.pending % 200;

        data.banknotes.$_100 = parseInt((data.pending / 100).toString());
        data.pending = data.pending % 100;

        data.banknotes.$_50 = parseInt((data.pending / 50).toString());
        data.pending = data.pending % 50;

        data.banknotes.$_20 = parseInt((data.pending / 20).toString());
        data.pending = data.pending % 20;

        data.will_dispense =
            data.banknotes.$_1000 > 0 ||
            data.banknotes.$_500 > 0 ||
            data.banknotes.$_200 > 0 ||
            data.banknotes.$_100 > 0 ||
            data.banknotes.$_50 > 0 ||
            data.banknotes.$_20 > 0;
    ...
    #internalCoinsChangeToReturn(change) {
        ...
        data.coins.$_10 = parseInt((change / 10).toString());
        data.pending = change % 10;

        data.coins.$_5 = parseInt((data.pending / 5).toString());
        data.pending = data.pending % 5;

        data.coins.$_2 = parseInt((data.pending / 2).toString());
        data.pending = data.pending % 2;

        data.coins.$_1 = parseInt(data.pending);
        data.pending = data.pending % 1;

        data.coins.$_50c = parseInt((data.pending / 0.5).toString());
        data.pending = data.pending % 0.5;
