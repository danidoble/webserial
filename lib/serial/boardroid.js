...
export class Boardroid extends Kernel {
    ...
    serialSetConnectionConstant(listen_on_port = 1) {
        ...
        arr[1] = this.hexMaker(this.decToHex((5 + listen_on_port).toString()));
    ...
    async dispense(
        ...
        if (null !== second_selection && (second_selection < 1 || second_selection > 80 || second_selection === selection)) throw new Error('Invalid second channel number');
        if (!sensor && (seconds === null || seconds <= 0 || seconds > 25.4)) throw new Error('Invalid time to dispense without sensor');

        if (retry) {
            this.__internal__.dispense.backup_dispense = {
                selection, second_selection, sensor, seconds
            };
        }

        selection += 9;
        const __channel1 = this.decToHex(selection);

        let __channel2 = '00';
        if (second_selection) {
            second_selection += 9;
            __channel2 = this.decToHex(second_selection);
        }
