...
export class Boardroid extends Kernel {
    ...
    async cardReaderDispense({channel = 1, second_channel = null, sensor = true, seconds = null, price = 0} = {}) {
        ...
        if (!sensor && (seconds === null || seconds <= 0 || seconds > 40.0)) throw new Error('Invalid time to dispense without sensor, must be between 0.1 and 40.0 seconds');
    ...
    async dispense(
        ...
        if (!sensor) {
            ...
            __type = this.decToHex(Math.round(seconds * 6.2));

            this.__internal__.dispense.custom_limit_counter = seconds;
        }
    ...
    async sendCustomCode({code = []} = {}) {
        ...
        const bytes = this.#boardroidCheckSum(code);
