...
import {open} from "./codes/locker.js";
...
export class Core extends Dispatcher {
        ...
        if (this.__internal__.serial.response.length === this.__internal__.serial.response.buffer.length) {
            ...
            if (this.__internal__.serial.response.buffer.length === double_length) {
                for (let jk = 14; jk < double_length; jk++) {
                    incoming[(jk - this.__internal__.serial.response.length)] = this.__internal__.serial.incoming[jk];
                }
            }

            this.__internal__.serial.incoming = new Uint8Array(0);
        }
        // else if(this.__internal__.serial.response.length > this.__internal__.serial.response.buffer.length){} // ... is needed?
    }
    ...
    async sendCustomCmd(cmd) {
        await this.appendToQueue(cmd, 'custom');
    }
    ...
    internalDispensingProcess() {
        ...
        this.__internal__.dispense.counter++;
    ...
    __emulate(data) {
        ...
        this.serialMessage(this.#bytesToLowerCase(data.code));
    }
