...
export class Locker extends Kernel {
  ...
  #serialLockerGetOpenCmd(cell = 1) {
    ...
    return this.serialLockerHexCmd(
      new Uint8Array([64, this.__internal__.device.listen_on_port, cell, timeHigh, timeLow])
    );
  }
  ...
  async dispense({ cell = 1 } = {}) {
    cell = this.#validateCell(cell);
    const code = this.#serialLockerGetOpenCmd(cell);
  ...
  async status({ cell = 1 } = {}) {
    ...
    return await this.appendToQueue(code, 'status');
  }
  ...
  async lightScan({ since = 0, until = 10 } = {}) {
    ...
    const code = this.#serialLockerGetLightScanCmd(since, until);
    return await this.appendToQueue(code, 'light-scan');
  }
