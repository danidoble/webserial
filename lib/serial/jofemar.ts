...
import { getSeconds, wait } from '../utils/utils';
  ...
  device: Internal['device'] & {
    ...
    channels: {
      verification: {
        ...
        current: number;
        channels: { selection: number; active: boolean }[];
      };
    };
    hex_number?: string;
    door_open: boolean;
...
interface TemperatureMessageSerial {
  traditional: number;
  ice_plus: number;
}
...
interface MessageSerial {
  ...
  additional: {
    ...
    speed?: string;
    enabled?: boolean;
    temperature?: number | TemperatureMessageSerial;
    scale?: string;
    full_hex?: string[];
    sign?: string | null;
    type_degrees?: string | null;
    decimal_point?: string | null;
    error?: string | null;
    degrees?: string | null;
    nsf_alarm?: number | null;
  };
}
...
export class Jofemar extends Kernel {
  declare __internal__: InternalJofemar;
  ...
  set startChannelVerification(value: number | string) {
    const start = parseInt(value as string);
  ...
  set listenOnChannel(channel: number) {
    channel = parseInt(channel as unknown as string);
  ...
  async #internalElevatorWaitingWithdrawal() {
    ...
    return new Promise((resolve) => {
      this.__internal__.dispense.elevator.locking_interval = setInterval(() => {
        ...
        this.__internal__.dispense.elevator.locking_time -= 1;
        if (this.__internal__.dispense.elevator.locking_time <= 0) {
          clearInterval(this.__internal__.dispense.elevator.locking_interval);
          this.__internal__.dispense.elevator.locking_interval = 0;
          resolve(true);
        }
      }, 1e3);
    });
  ...
  #internalMessageCommandExecuted(message: MessageSerial) {
    message.name = `ok`;
  ...
  #internalMessageKeyboardPushed(code: string, message: MessageSerial) {
    message.additional = {
      hex: code,
    ...
    const strCode = code.toString().toLowerCase();
    message.additional.ascii = ascii[strCode as keyof typeof ascii] ?? null;
  ...
  #internalMessageDoorEvent(code: string, message: MessageSerial) {
  ...
  #internalMessageChannelStatus(code: string[], message: MessageSerial) {
    message.no_code = 404;
    let machine: string | number = code[5] ?? null;
  ...
  #internalMessageProgramVersion(code: string[], message: MessageSerial) {
    message.no_code = 39;
  ...
  #internalMessageMachineFaults(code: string[], message: MessageSerial) {
    message.no_code = 39;
    ...
    if (bytes.length > 1 && bytes[0] !== '30') {
      ...
      for (const byte of bytes) {
        if (
          faults[byte as keyof typeof faults] &&
          message.additional.faults &&
          typeof message.additional.no_faults !== 'undefined'
        ) {
          message.additional.faults.push(faults[byte as keyof typeof faults] || 'Unknown fault');
  ...
  #internalMessageClockRegisters(code: string[], message: MessageSerial) {
    message.no_code = 40;
  ...
  #internalMessageMachineActivity(code: string[], message: MessageSerial) {
    message.no_code = 41;
    ...
    if (ascii !== '0') {
      ...
      if (ascii === 'T' && bytes.length === 4) {
        ...
        message.additional = {
          ...
          ascii,
          meaning: meanings[ascii as keyof typeof meanings] || 'Unknown',
        };
  ...
  #internalMessageCheckLanguage(code: string, message: MessageSerial) {
    const languages = {
      '30': 'Spanish',
      '31': 'English',
      '32': 'French',
    ...
    message.description = `The language is ${languages[code as keyof typeof languages] || 'unknown'}`;
    message.additional = {
      hex: code,
      language: languages[code as keyof typeof languages] || 'unknown',
    };
  ...
  #internalMessageCheckBeeper(code: string, message: MessageSerial) {
    message.no_code = 43;
    message.name = `Beeper`;
  ...
  #internalMessageCheckIsolationTray(code: string, message: MessageSerial) {
    message.no_code = 44;
  ...
  #internalMessageCheckEngineVoltage(code: string, message: MessageSerial) {
    message.no_code = 45;
  ...
  #internalMessageCheckPushOver(code: string, message: MessageSerial) {
    message.no_code = 46;
  ...
  #internalMessageCheckExtractorAfterDispense(code: string, message: MessageSerial) {
    message.no_code = 47;
  ...
  #internalMessageCheckStandbyAfterCollect(code: string, message: MessageSerial) {
    message.no_code = 48;
  ...
  #internalMessageCheckStandbyWithoutCollect(code: string, message: MessageSerial) {
    message.no_code = 49;
  ...
  #internalMessageCheckElevatorSpeed(code: string, message: MessageSerial) {
    message.no_code = 50;
  ...
  #internalMessageCheckExpirationByTemperature(code: string, message: MessageSerial) {
    message.no_code = 51;
  ...
  #internalMessageCheckTemperatureBeforeExpiration(code: string, message: MessageSerial) {
    message.no_code = 52;
  ...
  #internalMessageCheckExpirationAfter(code: string, message: MessageSerial) {
    message.no_code = 53;
  ...
  #internalMessageCheckTemperatureScale(code: string, message: MessageSerial) {
    message.no_code = 54;
  ...
  #internalMessageCheckMachineId(code: string[], message: MessageSerial) {
    message.no_code = 54;
  ...
  #internalMessageWorkingTemperature(code: string, message: MessageSerial) {
    message.no_code = 7;
  ...
  #internalMessageCurrentTemperature(code: string[], message: MessageSerial) {
    message.no_code = 8;
  ...
  #internalMessageByte0Case02(code: string[], message: MessageSerial, aux_machine: number = 128) {
    if (code[1]) {
      if (!message.additional.machine) {
        message.additional.machine = { hex: null, dec: null };
      }
    ...
    if (!(code[1] && code[2])) {
      ...
      message = this.#internalMessageCommandExecuted(message);
    } else {
      switch (code[2]) {
        case '54':
  ...
  #internalMessageByte0Case06(code: string[], message: MessageSerial) {
  ...
  #startStatusDispenseInterval() {
    ...
    this.__internal__.dispense.timeout = setTimeout(() => {
  ...
  async #reset(param: 'soldOut' | 'waiting' | 'machine') {
  ...
  async program(param1: number, param2: number) {
    return await this.appendToQueue(
  ...
  async checkData(type: number, aux: number = 0xff) {
    return await this.appendToQueue(
  ...
  override async sendCustomCode({ code = [] }: { code: any } = { code: [] }) {
    return await this.appendToQueue(
