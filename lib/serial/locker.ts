...
export class Locker extends Kernel {
  ...
  constructor({
    ...
    config_port,
    no_device = 1,
    device_listen_on_channel = 3,
    socket = false,
  }: ConstructorParamsLocker = {}) {
  ...
  override serialMessage(code: string[] | Uint8Array<ArrayBufferLike> | string | ArrayBuffer) {
    code = this.fixHexArray(code as unknown as string[] | number[]);
  ...
  #percentageOpenAll(dispensed: null | DispenseResponse[] = null) {
