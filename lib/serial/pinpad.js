...
export class PinPad extends Kernel {
  __pinPad__ = {
    ...
    operation: {
      ...
      reference: null,
        ...
        clientVoucher: '',
        consultDate: null,
        ignore: { counter: false, counterSale: false, isError92TRX: false, C93Global: false },
        bin: '',
        bin8: '',
    },
  };
  ...
  validateString(input) {
    ...
    if (input === undefined || input === null || input === '' || regex.test(input)) {
      return input;
    }
  ...
  async getPublicIp() {
        ...
        return new Promise((resolve) => {
            fetch('https://api.ipify.org?format=json')
                ...
                .then((data) => {
                    this1.__pinPad__.config.publicIP = data.ip || null;
  ...
  ab2str(buffer) {
    ...
    async print(voucherType) {
        ...
        if (!this.__pinPad__.operation.commerceVoucher.includes(':')) {
            this.__pinPad__.operation.commerceVoucher = this.RC4Decrypt(this.__pinPad__.config.RC4Key, this.__pinPad__.operation.commerceVoucher);
        }
        if (!this.__pinPad__.operation.clientVoucher.includes(':')) {
            this.__pinPad__.operation.clientVoucher = this.RC4Decrypt(this.__pinPad__.config.RC4Key, this.__pinPad__.operation.clientVoucher);
        }

        let voucher = this.__pinPad__.operation.commerceVoucher;
        if (voucherType !== 1) {
            voucher = this.__pinPad__.operation.clientVoucher;
        }
    ...
    async executeWS(url, data) {
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.setRequestHeader('data', encryptedKey || '');
            xhr.onerror = function () {
                this1.__pinPad__.operation.errors++;
                reject(new Error('EE19'));
            };
            xhr.send(encryptedData);
        });
    ...
    async getMerchantMOTO() {
        this.__pinPad__.operation.errors = 0;
        this.__pinPad__.operation.ignore.counter = false;
        this.__pinPad__.operation.ignore.counterSale = false;
        await this.validateObject({
            Ambiente: this.__pinPad__.config.defaultEnvironment,
            BIN: this.__pinPad__.operation.bin8,
            User: this.__pinPad__.config.username,
            Currency: this.__pinPad__.config.currency,
            Tx_OperationType: '',
        });
        const jsonRequest = await this.buildJson('9');
        const wsResponse = await this.executeWS(this.url + this.__pinPad__.constants.uris.merchant, jsonRequest);
        if (!wsResponse || wsResponse === '{}' || wsResponse.includes('Ha ocurrido un error al procesar su solicitud.')
        ) {
    ...
    async sendWSData(data, url, type) {
        ...
        return new Promise((resolve, reject) => {
                ...
                if (this.readyState === 4) {
                    ...
                    if (type === '2') {
                        ...
                        if (parsedResponse.respuesta == null || parsedResponse.respuesta === '0') {
                            ...
                            if (!this1.__pinPad__.operation.ignore.counter) {
                                this.dispatch('pinPad:response', this.responseText);
                                this1.__pinPad__.operation.ignore.counter = true;;
                            }
                        }
                    }
                }
            ...
            xhr.setRequestHeader('data', encryptedKey || '');
            xhr.onerror = function () {
                if (this1.__pinPad__.operation.errors < 1) {
                    this1.dispatch('pinPad:error', 'EE19');
                    this1.__pinPad__.operation.errors++;
                }
            };
            xhr.send(encryptedData);
    ...
    async getMerchant() {
        ...
        const dataMerchant = JSON.stringify({
            ...
            usuario: this.__pinPad__.config.username.toUpperCase(),
        ...
        await this.sendWSData(dataMerchant, this.url + this.__pinPad__.constants.uris.merchant, '2');
    }
  ...
  async finishCode92() {
    ...
    if (
      ...
      this.__pinPad__.buffer = '';
      const arr = this.stringToArrayBuffer(this.__pinPad__.operation.ignore.C93Global);
  ...
  async aboutPP(callback) {
    ...
    this.__pinPad__.about.serial = lineBuffer.substring(lineBuffer.indexOf('\x1CC') + 2, lineBuffer.indexOf('\x1CD'));
    this.__pinPad__.about.printer = lineBuffer.substring(
      lineBuffer.indexOf('\x1CF') + 2,
      lineBuffer.indexOf('\x1CG')
    );
    this.__pinPad__.about.brand = lineBuffer.substring(lineBuffer.indexOf('A00') + 3, lineBuffer.indexOf('\x1CB'));
    this.__pinPad__.about.model = lineBuffer.substring(lineBuffer.indexOf('\x1CB') + 2, lineBuffer.indexOf('\x1CC'));
    this.__pinPad__.about.appVersion = lineBuffer.substring(
  ...
  async initDUKPT(supportsDUKPT, hasDUKPTKeys) {
    ...
    await wait(4_000);
    if (this.__pinPad__.operation.ignore.isError92TRX) {
      this.__pinPad__.config.country = this.__pinPad__.config.country.toUpperCase();
      this.__pinPad__.config.idBranch = this.__pinPad__.config.idBranch.toUpperCase();
      this.__pinPad__.config.idCompany = this.__pinPad__.config.idCompany.toUpperCase();
      this.__pinPad__.config.password = this.__pinPad__.config.password.toUpperCase();
      this.__pinPad__.config.username = this.__pinPad__.config.username.toUpperCase();
    }
    ...
    const request = JSON.stringify({
      IPEK_REQUESTType: {
        business: {
          ...
          pwd: this.__pinPad__.config.password.toUpperCase(),
          user: this.__pinPad__.config.username.toUpperCase(),
        },
        terminal: {
    ...
    await this.sendWSData(request, this.url + this.__internal__.pinPad.urlKeysDUKPT, '1');
  }
  ...
  async readCard() {
    ...
    if (isEmpty(this.__pinPad__.operation.amount)) {
    ...
    command = command + FS + 'D' + this.formatNumber(this.__pinPad__.operation.amount, 2);
    ...
    if (this.validateAmount(this.__pinPad__.operation.amount) === false) {
      this.dispatch('pinPad:error', 'PPE02');
      return;
    }
    if (this.formatNumber(this.__pinPad__.operation.amount, 2) <= 0) {
      this.dispatch('pinPad:error', 'PPE02');
      return;
    }
    const arr = this.stringToArrayBuffer(command);
  ...
  consult() {
    this.__pinPad__.operation.errors = 0;
    let reference = this.__pinPad__.operation.reference.trim();
    if (isEmpty(reference)) {
      reference = '--';
        this.__pinPad__.operation.reference = reference;
    }
    ...
    this.validateObject(this.__internal__.pinPad.objDataConsulta)
      .then(async function () {
          this1.__pinPad__.operation.reference = reference;
        return await this1.validateReferenceWithBlank(this1.__pinPad__.operation.reference);
      })
      .then(async function () {
      ...
      .catch(function (error) {
        if (this1.__pinPad__.operation.errors < 1) {
          this1.dispatch('pinPad:error', error.message);
          this1.__pinPad__.operation.errors++;
        }
      });
  }
  ...
  processingEMV() {
    ...
    this.__pinPad__.config.read.POSEM = this.__pinPad__.buffer.substring(
      this.__pinPad__.buffer.indexOf('P93A') + 4,
      this.__pinPad__.buffer.indexOf('\x1CB')
    );
    if (this.__pinPad__.config.read.POSEM === '051' || this.__pinPad__.config.read.POSEM === '071') {
      this.__pinPad__.config.read.Chip = '1';
      this.__pinPad__.config.read.PIN = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CC') + 2,
        this.__pinPad__.buffer.indexOf('\x1CD')
      );
      this.__pinPad__.config.read.AppId = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CG') + 2,
        this.__pinPad__.buffer.indexOf('\x1CH')
      );
      this.__pinPad__.config.read.AppIdLabel = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CH') + 2,
        this.__pinPad__.buffer.indexOf('\x1CI')
      );
      this.__pinPad__.config.read.Arqc = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CF') + 2,
      ...
      this.__pinPad__.config.read.Tags = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CB') + 2,
        this.__pinPad__.buffer.indexOf('\x1CC')
      );
      this.__pinPad__.config.read.NB_ksn = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CK') + 2,
        this.__pinPad__.buffer.indexOf('\x1CM')
      );
      this.__pinPad__.config.read.NB_Data = this.__pinPad__.buffer.substring(
        this.__pinPad__.buffer.indexOf('\x1CD') + 2,
        this.__pinPad__.buffer.indexOf('\x1CE')
      );
      ...
      if (this.__pinPad__.config.read.POSEM === '022') {
        if (this.__pinPad__.buffer.includes('\x1CO')) {
          const subLine = this.__pinPad__.buffer.substring(this.__pinPad__.buffer.indexOf('P93A'));
          this.__pinPad__.operation.bin8 = subLine.substring(subLine.indexOf('\x1CO') + 2, subLine.indexOf(ETX));
  ...
  async validateSecondGenerate(command) {
    ...
    return new Promise(function (resolve, reject) {
      try {
        if (
          this1.__pinPad__.config.read.POSEM === '022' ||
          this1.__pinPad__.config.read.POSEM === '800' ||
          this1.__pinPad__.config.read.ReadCTLS === '1'
        ) {
          setTimeout(function () {
            resolve(false);
          }, 100);
  ...
  rePrint() {
    this.__pinPad__.operation.errors = 0;
    const this1 = this;
    this.validateOperationNumber(this.__pinPad__.operation.folio)
      .then(async function () {
        ...
        if (voucher === '' || voucher === null || voucher === undefined) {
          ...
          if (parsedResponse.voucher_comercio.includes(':')) {
            ...
            this1.__internal__.pinPad.companyVoucherGlobal = this1.RC4Decrypt(
              this1.__pinPad__.config.RC4Key,
  ...
  async internalConsultation(attempt) {
    ...
    this.__internal__.pinPad.objDataConsulta.User = this.__pinPad__.config.username;
    this.__internal__.pinPad.objDataConsulta.Pwd = this.__pinPad__.config.password;
    this.__internal__.pinPad.objDataConsulta.IdBranch = this.__pinPad__.config.idBranch;
    this.__internal__.pinPad.objDataConsulta.IdCompany = this.__pinPad__.config.idCompany;
    this.__internal__.pinPad.objDataConsulta.Country = this.__pinPad__.config.country;
  ...
  checkInMoto() {
    ...
    if (!this.validateReference(this1.__pinPad__.operation.reference)) {
      if (this1.__pinPad__.operation.errors < 1) {
        this1.dispatch('pinPad:error', 'bad-reference');
        this1.__pinPad__.operation.errors++;
      }
      return;
    }
    this.validateObject(this1.__internal__.pinPad.objDataCheckinMoto)
      ...
      .then(function () {
        return this1.formatAmount(this1.__pinPad__.operation.amount, 2);
      })
      .then(function (formattedAmount) {
        this1.__pinPad__.operation.amount = formattedAmount;
      ...
      .then(function (responseWS) {
        return new Promise(function (resolve) {
          ...
          if (companyVoucher !== null && companyVoucher.includes(':') === false) {
            companyVoucher = this1.RC4Decrypt(this1.__pinPad__.config.RC4Key, companyVoucher);
      ...
      .then(function () {
        return new Promise(function (resolve) {
          ...
          if (clientVoucher !== null && clientVoucher.includes(':') === false) {
            clientVoucher = this1.RC4Decrypt(this1.__pinPad__.config.RC4Key, clientVoucher);
  ...
  checkOut() {
    ...
    this.validateObject(this1.__internal__.pinPad.objDataCheckOut)
      .then(async function () {
        return await this1.validateOperationNumber(this1.__pinPad__.operation.folio);
      })
      .then(function () {
        return this1.validateAmount(this1.__pinPad__.operation.amount);
      })
      .then(function () {
        return this1.formatAmount(this1.__pinPad__.operation.amount, 2);
      })
  ...
  reauthorization() {
    ...
    this.validateObject(this.__internal__.pinPad.objDataReautorizacion)
      .then(function () {
        return this1.validateOperationNumber(this1.__pinPad__.operation.folio);
      })
      .then(function () {
        return this1.validateAmount(this1.__pinPad__.operation.amount);
      })
      ...
      .then(function (responseWS) {
        ...
        if (companyVoucher === '' || companyVoucher === null || companyVoucher === undefined) {
          this1.__pinPad__.operation.commerceVoucher = '';
        } else {
          if (respuestaParsed.voucher_comercio.includes(':')) {
            this1.__pinPad__.operation.commerceVoucher = respuestaParsed.voucher_comercio;
          } else {
            this1.__pinPad__.operation.commerceVoucher = this1.RC4Decrypt(
              this1.__pinPad__.config.RC4Key,
  ...
  validateRespCP(response) {
    ...
    return new Promise(function (resolve) {
      const parsedResponse = JSON.parse(response);
      this1.__internal__.pinPad.tokenRespOriginal = parsedResponse.number_tkn;
      let cdResponse = parsedResponse.cd_response;
      this1.__internal__.pinPad.responseTRX = parsedResponse.response;
      this1.__pinPad__.operation.folio = parsedResponse.foliocpagos;
      this1.__pinPad__.operation.authorization = parsedResponse.auth;
      ...
      this1.__internal__.pinPad.objFinishEMV.A = cdResponse;
      this1.__internal__.pinPad.objFinishEMV.B = '';
      this1.__internal__.pinPad.objFinishEMV.C = '';
      this1.__internal__.pinPad.objFinishEMV.D = '';
      this1.__internal__.pinPad.objFinishEMV.E = parsedResponse.emv_key_date == null ? '' : parsedResponse.emv_key_date;
      this1.__internal__.pinPad.objFinishEMV.F = parsedResponse.icc_csn == null ? '' : parsedResponse.icc_csn;
      this1.__internal__.pinPad.objFinishEMV.G = parsedResponse.icc_atc == null ? '' : parsedResponse.icc_atc;
      this1.__internal__.pinPad.objFinishEMV.H = parsedResponse.icc_arpc == null ? '' : parsedResponse.icc_arpc;
  ...
  sndVenta() {
    ...
    this.validateReference(this.__pinPad__.operation.reference);
  ...
  SndVentaTkn() {
    ...
    this.__pinPad__.operation.ignore.counter = false;
    this.__pinPad__.operation.ignore.counterSale = false;
    this.__pinPad__.config.country = this.__internal__.pinPad.objDataVentaTkn.Country.toUpperCase();
    this.__pinPad__.config.idBranch = this.__internal__.pinPad.objDataVentaTkn.IdBranch.toUpperCase();
    this.__pinPad__.config.idCompany = this.__internal__.pinPad.objDataVentaTkn.IdCompany.toUpperCase();
    this.__pinPad__.config.password = this.__internal__.pinPad.objDataVentaTkn.pwd.toUpperCase();
    this.__pinPad__.config.username = this.__internal__.pinPad.objDataVentaTkn.User.toUpperCase();
    ...
    if (!this.validateReference(this.__pinPad__.operation.reference)) {
      return;
    }
  ...
  CheckinBanda() {
    ...
    this.__pinPad__.config.country = this.__internal__.pinPad.objDataCheckinBanda.Country.toUpperCase();
    this.__pinPad__.config.idBranch =
      this.__internal__.pinPad.objDataCheckinBanda.IdBranch.toUpperCase();
    this.__pinPad__.config.idCompany =
      this.__internal__.pinPad.objDataCheckinBanda.IdCompany.toUpperCase();
    this.__pinPad__.config.password = this.__internal__.pinPad.objDataCheckinBanda.pwd.toUpperCase();
    this.__pinPad__.config.username = this.__internal__.pinPad.objDataCheckinBanda.User.toUpperCase();
    if (!this.validateReference(this.__pinPad__.operation.reference)) {
      return;
    }
  ...
  Cancelacion() {
    const this1 = this1;
    this1.__pinPad__.operation.errors = 0;
    this.validateObject(this1.__internal__.pinPad.objDataCancelacion)
      .then(function () {
        return this1.validateOperationNumber(this1.__pinPad__.operation.folio);
      })
      .then(function () {
        return this1.ValidarNumAuth(this1.__pinPad__.operation.authorization);
      })
      .then(function () {
        return this1.validateAmount(this1.__pinPad__.operation.amount);
      })
  ...
  GetVoucherCliente() {
    if (!this.__pinPad__.operation.ignore.counter) {
      const this1 = this;
      this.dispatch(
      ...
      this.__pinPad__.operation.ignore.counter = true;;
    }
  }
  ...
  GetVoucherComercio() {
    if (!this.__pinPad__.operation.ignore.counter) {
      ...
      this.dispatch(
        ...
        JSON.stringify({
          RESPUESTA: 'ok',
          voucher_comercio: this1.__pinPad__.operation.commerceVoucher,
        })
      );
      this.__pinPad__.operation.ignore.counter = true;;
    }
  ...
  async e_cancelReadCard() {
    // this.environment = "qa"; ????
    //this.__pinPad__.about.model = message;
    this.__pinPad__.operation.ignore.counter = false;
      ...
      if (this.__pinPad__.buffer.length > 11) {
        if (this.__pinPad__.buffer.includes('P59A00')) {
          if (this.__pinPad__.operation.clientVoucher !== '') {
            ...
            this.__internal__.pinPad.respuestaWS = '{ "RESPUESTA": "ok", "ERROR": ""}';
            if (this.__pinPad__.operation.errors < 1) {
  ...
  async receiveCancelReadCard() {
    if (this.__pinPad__.buffer.length > 10) {
      if (this.__pinPad__.buffer.includes('E93A10') && this.__pinPad__.operation.errors < 1) {
    ...
    async buildJson(type) {
        ...
        if (type === 2) {
            return JSON.stringify({
                ...
                reference: this.__pinPad__.operation.reference.toUpperCase(),
            });
        }
        if (type === 3) {
            JSON.stringify({
                VMCAMEXMCANCELACION: {
                    ...
                    transacction: {
                        amount: this.__pinPad__.operation.amount,
                        auth: this.__pinPad__.operation.authorization.toUpperCase(),
                        crypto: '2',
                        no_operacion: this.__pinPad__.operation.folio,
                        usrtransacction: this.__pinPad__.config.username.toUpperCase(),
        ...
        if (type === 4) {
            return JSON.stringify({
                VMCAMEXMREVERSO: {
                    ...
                    transacction: {
                        amount: this.formatNumber(this.__pinPad__.operation.amount, 2),
                        no_operacion: this.__pinPad__.operation.folio,
                        auth: this.__pinPad__.operation.authorization.toUpperCase(),
                        tracks: '',
                        usrtransacction: this.__pinPad__.config.username.toUpperCase(),
        ...
        if (type === 5) {
            if (this.__pinPad__.config.read.Chip.toString() === '0') {
                return JSON.stringify({
                    VMCAMEXBCHECKIN: {
                        business: {
                            id_company: this.__pinPad__.config.idCompany.toUpperCase(),
                ...
                VMCAMEXMCHECKIN: {
                    business: {
                        ...
                        pwd: this1.RC4Encrypt(
                            ...
                            this.__pinPad__.config.password.toUpperCase()
                        ),
                    },
                    transacction: {
