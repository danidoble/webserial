...
export class Jofemar extends Kernel {
    ...
    set listenOnPort(port) {
        ...
        this.__internal__.device.hex_number = (128 + port).toString(16);
    }
    ...
    async collect() {
        const bytes = ['02', '30', '30', '81', '4E', 'FF', 'FF'];
        return await this.#addToQueueWithChecksum(bytes, 'collect');
    }

    async #reset(param) {
        const bytes = ['02', '30', '30', '81', '52', param, 'FF'];
    ...
    async status() {
        const bytes = ['02', '30', '30', '81', '53', 'FF', 'FF'];
        return await this.#addToQueueWithChecksum(bytes, 'status');
    }

    async #lights(param) {
        const bytes = ['02', '30', '30', '81', '4C', param, 'FF'];
    ...
    async program(param1, param2) {
        const bytes = ['02', '30', '30', '81', '50', param1, param2];
    ...
    async programWorkingTemperature({degrees = 0.5} = {}) {
        ...
        if (isNaN(degrees) || degrees < min || degrees > limit || degrees % 0.5 !== 0) throw new Error('Invalid degrees, must be a multiple of 0.5 and between 0.5 and 32');
    ...
    async programTemperatureBeforeExpiration({degrees = 0.5} = {}) {
        degrees = parseFloat(degrees);
        if (isNaN(degrees) || degrees < 0.5 || degrees > 30 || degrees % 0.5 !== 0) throw new Error('Invalid degrees, must be a multiple of 0.5 and valid range is 0.5 to 30');
    ...
    async programVoltageEngine({selection = 1, voltage = 5} = {}) {
        ...
        selection = parseInt(selection);
        if (isNaN(selection) || selection < 1 || selection > this.__device.channels.verification.end) throw new Error(`Invalid selection, valid range is 1 to ${this.__device.channels.verification.end}`);
        ...
        const bytes = ['02', '30', '30', '81', '47', param1, param2];
    ...
    async programPushOverProducts({selection = 1, enable = true} = {}) {
        ...
        const bytes = ['02', '30', '30', '81', '4F', param1, param2];
    ...
    async programChannelRunningAfterDispense({selection = 1, seconds = 0} = {}) {
        ...
        if (isNaN(selection) || selection < 1 || selection > this.__device.channels.verification.end) throw new Error(`Invalid selection, valid range is 1 to ${this.__device.channels.verification.end}`);
        if (isNaN(seconds) || seconds < 0 || seconds > 10 || seconds % 0.1 !== 0) throw new Error('Invalid seconds, valid range is 0.0 to 10.0 with a step of 0.1');
        const param1 = 109 + selection;
    ...
    async #displayConfig(type = '80', param2 = []) {
        const bytes = ['02', '30', '30', '81', '44', type, ...param2];
    ...
    async programClock({date = new Date()} = {}) {
        if (!(date instanceof Date)) throw new Error('Invalid date, must be an instance of Date');
        const bytes = ['02', '30', '30', '81', '72', ...this.#makeTimeFormat(date)];
    ...
    async eventsConfig({event = null, enable = true} = {}) {
        ...
        const value = enable ? '31' : '30';
        const bytes = ['02', '30', '30', '81', '41', event, value];
    ...
    async sendCustomCode({code = []} = {}) {
        ...
        return await this.#addToQueueWithChecksum(code, 'custom');
    }
