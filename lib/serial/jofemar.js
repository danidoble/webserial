...
export class Jofemar extends Kernel {
    ...
    #internalMessageDoorEvent(code, message) {
        ...
        if (code === '4f') {
            ...
            message.additional.door.open = true;
    ...
    #internalMessageClockRegisters(code, message) {
        ...
        const date = bytes.map(hex => String.fromCharCode(this.hexToDec(hex))).join('');
        const [time, date1] = date.split(' ');
        ...
        if (code[1] && !code[2]) {
            switch (code[1]) {
                ...
                case '45':
                    ...
                    message.no_code = 23;
                    this.#setNotDispensed();
                    this.#jofemarError({type: 'eeprom-writing-error', severity: 'extreme'});
                    break;
                case '46':
                ...
                case '4a':
                    ...
                    message.no_code = 28;
                    this.#setNotDispensed();
                    this.#jofemarError({type: 'channels-power-consumption-detector-faulty', severity: 'extreme'});
                    break;
                case '4b':
    ...
    async dispense(
        ...
        if (isNaN(selection) || selection < 1 || selection > 109) throw new Error('Invalid selection');
    ...
    #dispatchResettingErrors() {
        this.dispatch('reset:errors', {
            ...
            duration: this.__device.type === 'esplus' ? getSeconds(25) : getSeconds(40),
            started_at: new Date(),
        });
    }
