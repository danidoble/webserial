...
export class Jofemar extends Kernel {
  ...
  #internalGetTrayChannel(selection) {
    ...
    const tray = (parseInt(selection.substring(0, 2)) + 128).toString(16).padStart(2, '0');
    const channel = (parseInt(selection.substring(2, 3)) + 128).toString(16).padStart(2, '0');
    return { channel, tray };
  }
  ...
  async programWorkingTemperature({ degrees = 0.5 } = {}) {
    ...
    if (isNaN(degrees) || degrees < min || degrees > limit || degrees % 0.5 !== 0)
  ...
  async programVoltageEngine({ selection = 1, voltage = 5 } = {}) {
    ...
    selection = parseInt(selection);
    if (isNaN(selection) || selection < 1 || selection > this.__device.channels.verification.end)
  ...
  async getVoltageEngine({ selection = 1 } = {}) {
    selection = parseInt(selection);
    if (isNaN(selection) || selection < 1 || selection > 126)
